# 强制规则（AI 行为约束，优先级仅次于 Flow Gate）

> 本文件规定 AI 的**行为约束**（交互、Git、宏、路径、迭代上限）。
> 流程的**顺序与门禁纪律**（必须按阶段/步骤执行、不可跳过门禁）由 `gates/FLOW_GATE_RULES.md` 统一规定，二者互补、不重复。

1. **交互询问规则** — 仅调试启动阶段一次性采集全部参数写入配置；运行中仅 3 类固定场景触发 `🛑人工暂停`（设备断电重启、Keil 断点调试、迭代上限求助），其余全程静默。暂停场景细则见 `refs/pause-scenarios.md`。
