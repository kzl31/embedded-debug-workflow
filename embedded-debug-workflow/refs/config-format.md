...
> `--init` 不扫描工作区，也不在开始时询问配置项；缺少配置时固定生成两个项目。
> 用户可自行增加、删除或重排 `projects` 元素；后续始终以数组实际内容为准，`project_count`
> 仅是生成时记录，不参与运行时判断。下一阶段会对每个项目分别询问执行模式，允许例如
> 项目 0 选择 `full`、项目 1 选择 `none`、项目 2 选择 `compile_only`。
> 默认值为：Keil `C:\Keil_v5\UV4\UV4.exe`、串口 `COM1/256000/8N1`、
> 下载器 `JLink/COM1`；工程路径和工程文件保留为空。用户必须根据实际环境修改这些值。
