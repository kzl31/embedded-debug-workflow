# /kzl by（编译） · byxz（编译下载） — 编译 / 编译+下载固件

> 本命令处理两类触发：
>   - `/kzl by`  / `/kzl 编译`      → **仅编译**（不下载固件）
>   - `/kzl byxz` / `/kzl 编译下载` → 编译 + 下载固件
>   （兼容旧别名：`/kzl build` 等效 `/kzl byxz`）
>
...
### ⛔ 前置：Flow Gate 预检（必须已初始化）

```yaml
type: flow_gate_precheck
说明: 按 SKILL.md 的 Flow Gate 门禁预检步骤执行，确认已初始化且当前阶段允许编译/下载
```
...
question: |
  ...
  2) 是否下载固件？（/kzl by 默认不下载，/kzl byxz 默认下载；与默认不同请说明）
