...
5. **提交信息使用中文**：所有 `git commit -m` 提交信息必须使用简洁、明确的中文，说明本次提交的
	目的或内容；允许保留必要的技术标识、文件名、函数名、错误码和协议名。禁止使用无意义的
	`update`、`fix`、`tmp`、`wip` 等纯英文或无描述提交信息。
...
每一轮即将修改源码、工程配置或其他受流程管理文件前，必须先 `git commit` 当前状态作为本轮回退基线；提交完成后再进行本轮修改。每轮只允许建立一个修改前基线，提交信息应使用中文并包含轮次或步骤。临时 CHESHI 调试代码可以提交到本地调试分支，但不得推送，也不得与正式业务修复混淆。
...
git add 目标修复文件.c
git commit -m "修复TTL通信故障：纠正PDU帧长度计算偏移"
```
