...
配置文件名固定为 `embedded-debug-config.json`，存放在项目根目录（即 `--init` 指定的目录）。
...
def resolve_config_path(path_or_dir: str | Path | None = None) -> Path:
    ...
    规则：
    - 路径为空 → 自动搜索（当前目录 → 父目录 → skill data/）
    - 路径指向已存在的文件 → 直接返回
    - 路径指向目录或不存在 → 当做目录，在其下找 {CONFIG_FILENAME}
    """
...
def get_config_path(save_dir: str | Path | None = None) -> Path:
    """返回配置文件的完整路径（别名，用于 save_config）。"""
    if save_dir:
        return Path(save_dir).resolve() / CONFIG_FILENAME
...
def load_config(save_dir: str | Path | None = None) -> dict[str, Any]:
    ...
    save_dir 可以是：
    - None → 自动搜索
    - 目录路径 → 在该目录下找 {CONFIG_FILENAME}
    - 文件路径 → 直接读取该文件
    """
    if save_dir is not None:
        p = Path(save_dir)
        path = p if p.is_file() else (p / CONFIG_FILENAME)
    else:
