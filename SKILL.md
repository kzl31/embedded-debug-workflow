...
步骤 1 的 Python 脚本在终端中完成全部初始化参数选择：

- 配置版本：输入 `1` 按当前配置进行，输入 `2` 已修改配置并按最新进行；
- 每个项目模式：输入 `1` 不编译不下载、`2` 仅编译、`3` 编译下载、`4` 编译下载监听。

脚本保存结果后，引擎自动同步 `projectModes` 和执行能力。旧的 AI 配置确认与逐项目询问步骤已删除，
不得再使用提问工具重复询问。
...
- `status = "awaiting_user"` → 触发人工暂停（`wait_user`），处理用户后 `--wake` 恢复。报告步骤（step 20）失败时同样进入 `wait_user` 人工暂停，不会自动重试覆盖，处理完 `--wake` 恢复。
...
4. **只有到达流程完成步骤才可以停止**
5. **引擎调用前展示进度**：每次准备调用流程引擎前，先依据上一次引擎返回的 `seq`、`phase`、
  `what`、`status` 和已完成动作，向用户发送一条独立的简短进度消息。消息必须包含上一动作的
  简要结果和即将调用的步骤，格式必须严格为 `> 内容`（行首一个 `>` 加一个空格）；首次调用使用
  `> 初始化流程，准备进入步骤 1`。调用引擎后不得重复输出
  `user_display`，也不得把引擎 JSON 当作用户已经看到进度的证明。

7. **单次引擎调用约束**：进度展示发生在引擎调用之前，不为展示进度重复调用引擎。引擎返回后只
  按 JSON 决策执行当前动作；若动作完成后需要再次调用引擎，必须先展示下一条进度消息。进度消息
  必须严格使用 `> 内容`，不得扩写分析、日志、根因或结论。

8. **AI 步骤完整输出**：当引擎返回 `status = "awaiting_ai"` 时，必须完整读取并执行
  `ai_step.instructions`，不得只依据 `message` 或 `what` 简化、跳过当前步骤。完成后必须按
  `next_action` 提交 `--ack success`；无法完成或证据不足时提交 `--ack failure`，不得停留在
  当前步骤或伪造完成。仅当 `ai_progress_display=true` 时读取并展示 `progress.current`；该配置
  为 `false` 时引擎不返回 `progress` 与 `user_display`，但不影响 AI 步骤本身的执行。
...
**条件语法**：`section.field <op> 值`，如 `debugLoopInfo.compileRetryCount < {meta.max_retry}`、`projectInfo.hasSerialProjects == true`（重试上限来自 `flow.yaml` 的 `meta.max_retry`，当前为 2；编译与下载统一引用该值）；
支持 `== != < > <= >=`，数值/字符串自动判断。
