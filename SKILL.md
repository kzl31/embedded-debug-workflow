...
4. **只有到达流程完成步骤才可以停止**
5. 引擎 JSON 含 `user_display` 时，每个到达的流程步骤至少向用户输出一次进度，且只原样输出
  `user_display.text`；不得用“内容描述”等占位文本替代，也不得自行扩写分析、日志、根因或结论
6. 配置项 `ai_progress_display=false` 时，引擎不会返回 `user_display`，AI 不输出流程进度；
  `user_display.loop` 存在时，其循环次数和原因已经合并进 `user_display.text`，仍只原样输出该字段
7. **工具返回不等于用户展示**：`status=awaiting_progress` 时，必须先发送一条独立、用户可见且只含
  `user_display.text` 的消息；发送后才能执行 `next_action`（`--ack-progress`）。禁止看到 JSON 后直接
  连续调用命令、编辑或分析工具；未确认进度时引擎不会执行当前步骤。每次跳转或重试到达步骤都重新门禁。
